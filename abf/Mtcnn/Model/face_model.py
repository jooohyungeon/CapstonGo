...
class FaceModel(object):
    def __init__(self, args, ctx):
        ...
        if args.det == 0:
            detector = MtcnnDetector(model_folder=mtcnn_path, ctx=self.ctx, num_worker=1, accurate_landmark=True,
    ...
    def get_input_nd(self,face_img):
        aligned = []
    ...
    def get_input_new(self, face_img):  # can detect multiple faces from the image
        # made this because get_input is not programmed to return boxes, and points. also it returns only 1 person's
        # aligned face image.
        aligned = []
        t1 = time.time()
        ret = self.detector.detect_face(face_img, det_type=self.args.det)
